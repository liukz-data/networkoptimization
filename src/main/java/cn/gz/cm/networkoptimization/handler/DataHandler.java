...
public class DataHandler {
    ...
    private Accumulator accumulatorTakeValue = new Accumulator();
        ...
        Future<String> resultWriteThreadPool = writeThreadPool.submit(new Callable<String>() {
            ...
            public String call() {
                    ...
                    try {
                        ...
                        Future<String> result = writeThreadPool.submit(new Callable<String>() {
                            ...
                            public String call() {
                                ...
                                logger.debug("Data accumulation：" + pool.size());
                                logger.debug("take:" + line);
                                accumulatorTakeValue.accumulation();
                ...
                for (Future<String> listResultSend : listResultSends) {
                    try {
                        logger.debug(listResultSend.get());
                    } catch (Exception e) {
                        ExceptionHandler.exceptionHandler(e,logger);
                        e.printStackTrace();
                    }
    ...
    public boolean waitComplete() throws ExecutionException, InterruptedException {
       ...
       // logger.info("Kafka Producer Pool has stop!!!");
        logger.info("The number of times the AccumulatorTakeValue is called: "+accumulatorTakeValue.getValue());
        logger.info("The number of times the AccumulatorCallBackValue is called: "+callBack.getAccumulatorCallBackValue().getValue());
        logger.info("The number of times the AccumulatorCallBackValueError is called: "+callBack.getAccumulatorCallBackValueError().getValue());
        return true;
    }
